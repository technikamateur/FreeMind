...
python3 /etc/freemind/fmmain.py backupready 1
...
then
  ...
  python3 /etc/freemind/fmmain.py error 15
fi
# checking HDD online
...
then
  ...
  python3 /etc/freemind/fmmain.py backupready 0
...
lastupdate="$(python3 /etc/freemind/fmmain.py update lastupdate)"
...
if [ $updatedone == true ]
then
  python3 /etc/freemind/fmmain.py update system
else
  python3 /etc/freemind/fmmain.py error 14
fi
# getting free space and sending to database
i=0
for hdd in "sda2" "sdb1" "sdc1" "sdd1"
do
  i=$i+1
  memhdd="$(df -h | grep $hdd | awk '{print $2}')"
  python3 /etc/freemind/fmmain.py memtotal $i $memhdd
done
i=0
for hdd in "sda2" "sdb1" "sdc1" "sdd1"
do
  i=$i+1
  memhdd="$(df -h | grep $hdd | awk '{print $3}')"
  python3 /etc/freemind/fmmain.py memis $i $memhdd
done
# Is a Hard Disk near limit? Filedrive? Then stop samba!
